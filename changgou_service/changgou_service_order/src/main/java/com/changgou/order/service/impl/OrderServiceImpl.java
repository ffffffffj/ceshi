...
//        int a = 1/0;
    ...
    public void closeOrder(String orderId) {
        /**
         * 1.根据订单id查询mysql中的订单信息,判断订单是否存在,判断订单的状态
         * 2.基于微信查询订单信息
         * 3.如果当前订单的支付状态为已支付,则进行数据的补偿操作(mysql)
         * 4.如果当前订单的支付状态为未支付,则修改mysql中的订单信息,新增订单日志,恢复商品的库存
         */
    ...
    public void batchSend(List<Order> orders) {
        ...
        for (Order order : orders){
            ...
            if (order.getShippingCode() == null || order.getShippingName() == null){
                throw new RuntimeException("请输入运单号或物流公司名称");
            }
        }
    ...
    public void confirmTask(String orderId, String operator) {
        ...
        order.setOrderStatus("2");//已送达
        order.setConsignStatus("3");//已完成
        order.setConsignTime(new Date());
        order.setUpdateTime(new Date());
        orderMapper.updateByPrimaryKeySelective(order);
